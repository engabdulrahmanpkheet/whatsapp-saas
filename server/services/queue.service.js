 ...
 *
 * Durable, MongoDB-backed campaign queue (v3).
 *
 * v3 additions:
 *  - Warm-up gradient: respects campaign.warmup.{enabled, day1, day_step, started_at}
 *    Effective daily cap = min(license.max_messages_per_day, warmup_cap)
 *  - Returns campaign.media + campaign.variations to the worker
 */
...
async function reclaimStale(campaignId) {
  ...
  await Campaign.updateOne(
    ...
    {
      arrayFilters: [
        { 'stuck.status': 'processing', 'stuck.locked_at': { $lt: cutoff } },
      ],
    }
  );
}

/** Compute today's effective daily cap (lic limit ∩ warm-up ramp). */
function effectiveDailyCap(campaign, license) {
  const licCap = license.max_messages_per_day || 0; // 0 = unlimited
...
async function claimNextJob(licenseKey, campaignId) {
  ...
  const claimed = await Campaign.findOneAndUpdate(
    ...
    {
      new: true,
      arrayFilters: [{ 'next.status': 'pending' }],
    }
  );
  ...
  const contact = claimed.contacts.find(
    (c) =>
      c.status === 'processing' &&
      c.locked_at &&
      c.locked_at.getTime() === now.getTime()
  );
